...
const { $swal }: any = useNuxtApp()
 ...
 // เรียกใช้ useCookie สำหรับเก็บข้อมูลลง cookie
 const token = useCookie(
        'token',
        {
           // กำหนดอายุของ cookie ให้เก็บไว้ 24 ชั่วโมง
           maxAge: 60 * 60 * 24
        }
    )
 ...
 const userData = useCookie(
        'userData', {
            // ตั้งค่า cookie ให้หมดอายุหลังจาก 24 ชั่วโมง
            maxAge: 60 * 60 * 24
        }
    )
...
// เรียกใช้ useFormRule สำหรับ validate form
    ...
    const submitForm = () => {
        ...
        if(
            ...
            // const SPRING_API_IMAGE = config.public.urlimage
                    ...
                    onResponse({ request, response, options }) {
                        ...
                        if(response.status == 401){
                           ...
                           token.value = response._data.data.token
                            ...
                            // userData.value = JSON.stringify(response._data.data)
